...
public class BigramWithUnigramPriorLanguageModel implements LanguageModel {
     ...
     */
    public BigramWithUnigramPriorLanguageModel(HolbrookCorpus corpus) {
        ...
        for (Sentence sentence : corpus.getData()) { // iterate over sentences
            for (int i = 0; i < sentence.size() - 1; i++) {
                ...
                if (aDouble == null || aDouble <= 0d) {
                    Double prob = calculateBigramPriorSmoothingProbability(sequentWord, precedingWord, words, 1d);
                    scores.put(s, prob);
                }
            }
        }
    ...
    private Double count(String word, Collection<String[]> set) {
     ...
     public Double calculateBigramPriorSmoothingProbability(String sequentWord, String precedingWord, Collection<String[]> set, Double k) {
    ...
    public double score(List<String> sentence) {
        ...
        for (int i = 0; i < sentence.size() - 1; i++) {
            ...
            if (aDouble != null && aDouble > 0d) {
                ...
                double v = Math.log(calculateBigramPriorSmoothingProbability(sentence.get(i + 1), sentence.get(i), words, 1d));
                score *= v;
                scores.put(s, v);
            }
        }
